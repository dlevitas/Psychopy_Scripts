...
#Create GUI:
expName = "CMNT"
expInfo = {"participant ID": "", "run": ""} #Condition Files: F1, F2, M1, or M2
...
text2 = visual.TextStim(win = win, text = '', height = 0.1, pos = (0,0), color="White")
...
image = visual.ImageStim(win = win, pos = (0,0), size = (0.1,0.1), image = os.path.join(cwd + "/Stimuli/gray.png"))
rect = visual.Rect(win=win, width = 0.3, height = 0.2, pos = (0,0))
    
#Run Instruction Page:
text.text = '\t\tREMEMBER:\n\t\tAnswer each question,\n\t\t"Which should I pick?"\n\t\tusing the LEFT button key for \n\t\tOPTION 1 and the RIGHT\n\t\tbutton key for OPTION 2.\n\n\t\t Please wait for the scanner'
text.height = 0.1
text.draw()
win.flip()
while True:
...
for i in xrange(block_start, block_end):
    ...
    if stim_df["speaker"][i] == "Computer":
        ...
        image.image = os.path.join(cwd + "/Stimuli/gray.png")
        image.pos = (-0.1,0)
        ...
        image.image = os.path.join(cwd + "/Stimuli/blue.png")
        image.pos = (-0.1,0)
        image.size = (text_box_length, 0.35)
    ...
    if chatBuddy in ["Domenick","Harrison"]:
        text1.text = "Which should he pick?"
        text1.pos = (0,0.6)
        text1.height = 0.1
        text1.setAutoDraw(True)
    else:
        text1.text = "Which should she pick?"
        text1.pos = (0,0.6)
        text1.height = 0.1
        text1.setAutoDraw(True)
    ...
    while timer.getTime() < 4 + t3:
        if event.getKeys(keyList = ["escape"]):
            core.quit()
        """    
        event_press = event.getKeys(keyList = ["1","2"])
    ...
    if stim_df["speaker"][i] == "Computer": 
        ...
        if acc == 1:
            ...
            speaker.draw()
            text.draw()
            image.image = os.path.join(cwd + "/Stimuli/green.png")
            image.size = (text.boundingBox[0]/width*2 + 0.1, text.boundingBox[1]/height*2 + 0.1)
            image.pos = (0.1, 0.3)
